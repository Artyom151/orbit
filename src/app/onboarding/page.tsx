      ...
      reader.onloadend = async () => {
        ...
        try {
          ...
          if (result.colors && result.colors.length === 2) {
            ...
            // Keep default gradient on failure
            setGradientColors(['#a855f7', '#6366f1']);
        } finally {
    ...
    <div className="flex min-h-screen flex-col items-center justify-center bg-background p-8">
      <div className="w-full max-w-4xl mx-auto">
              ...
              <div className="relative h-32 w-full">
                ...
                {isExtractingColors && (
                    <div className='absolute inset-0 bg-black/50 flex items-center justify-center'>
                        <Wand2 className='text-white w-8 h-8 animate-pulse' />
                    </div>
                )}
              </div>
              <div className="p-4">
                ...
                <div className="pt-2">
                  ...
                  <p className="text-muted-foreground">@{username || "your_username"}</p>
        ...
        <div className="text-center mt-12">
          <Button size="lg" onClick={handleFinish} disabled={isSubmitting || isExtractingColors || !name || !username}>
            {(isSubmitting || isExtractingColors) && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
            {isSubmitting ? 'Saving...' : isExtractingColors ? 'Analyzing colors...' : 'Finish Setup & Enter Orbit'}
          </Button>
        </div>
      </div>
    </div>
