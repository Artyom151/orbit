...
export const acceptConnection = async (db: Database, currentUserId: string, otherUserId: string, currentUserData: {uid: string; displayName?: string | null; photoURL?: string | null, username: string}) => {
    const myConnectionRef = ref(db, `connections/${currentUserId}/${otherUserId}`);
    const theirConnectionRef = ref(db, `connections/${otherUserId}/${currentUserId}`);
