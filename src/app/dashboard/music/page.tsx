...
export default function MusicPage() {
    ...
    const { toast } = useToast();
    const { selectTrack, setIsFullScreenPlayerOpen } = useMusicPlayer();

    useEffect(() => {
        setIsFullScreenPlayerOpen(true);
        return () => setIsFullScreenPlayerOpen(false);
    ...
    const handlePlayFavorites = () => {
        if (favoriteTracks.length > 0) {
            const sortedFavorites = [...favoriteTracks].sort((a, b) => b.favoritedAt - a.favoritedAt);
            selectTrack(sortedFavorites[0], 0, sortedFavorites);
        }
    }
    ...
    const sortedFavoriteTracks = useMemo(() => {
        return [...favoriteTracks].sort((a, b) => b.favoritedAt - a.favoritedAt);
    }, [favoriteTracks]);
                        ...
                        {!isGeneratingMix && mixTracks.length === 0 && (
                             <div className="pb-32">
                                ...
                                {loadingFavorites ? (
                                    ...
                                    <div className="space-y-2">
                                    {sortedFavoriteTracks.map((track, index) => (
                                        <TrackItem 
                                            key={track.trackId}
                                            track={track}
                                            onSelect={() => selectTrack(track, index, sortedFavoriteTracks)}
                                            isFavorite={true}
                                            onToggleFavorite={() => handleToggleFavorite(track)}
                                        />
                                    ))}
